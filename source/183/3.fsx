module Parser = 
  ...
  *)
  ...
  let exprPwr rbpw parser =
    let rec expr left =
      ...
      | Some token when rbpw < (parser |> getBindingPower token) -> 
        ...
        let led = parser.Left.[type']
        led token left parser |> expr

      | _ -> left

    let tok = parser |> skipCurrent
    expr (parser.Null.[parser.Type tok] tok parser)
  ...
  let matchError () = 
    exn "Match pattern failed"

  let create<'a, 'b, 'c when 'c : comparison> type' pos string = {
    Input = ref []
    Lines = None
...
let infix typ pwr p =
  let infix tok left (p:P) =
    Binary(tok |> toBinaryOp, left, p |> P.exprPwr pwr)
...
let prefix typ p =
  let prefix tok (p:P) =
    Unary(tok |> toUnaryOp, p |> P.exprPwr 70)
