...
type arithmetic = Add | Subtract | Multiply | Divide | Exponentation
...
let compare lhs rhs =
    ...
    | Int l, Double r -> (double l).CompareTo(r)
    | Double l, Int r -> l.CompareTo(double r)
...
let rec eval (vars:Dictionary<identifier,value>) (expr:expr) =
    match expr with
    | Literal x -> x
    | Var identifier -> vars.[identifier]
    | Element(_,_) -> raise (System.NotImplementedException())
    | Func(call) -> invoke vars call
    | Neg x -> arithmetic (eval vars x) Multiply (Int(-1))
    | Arithmetic(l,op,r) -> arithmetic (eval vars l) op (eval vars r)
    | Comparison(l,op,r) -> comparison (eval vars l) op (eval vars r)
    | Logical(l,op,r) -> logical (eval vars l) op (eval vars r)
...
and arithmetic lhs op rhs =
    ...
    | Add, Doubles (l,r) -> Double(l + r)
    | Add, (String l, String r) -> String(l + r)
    | Subtract, (Int l,Int r) -> Int(l - r)
    | Subtract, Doubles (l,r) -> Double(l - r)
    | Multiply, (Int l,Int r) -> Int(l * r)
    | Multiply, Doubles (l,r) -> Double(l * r)
    | Divide, (Int l,Int r) -> Int(l - r)
    | Divide, Doubles (l,r) -> Double(l - r)
...
and invoke vars (Call(mi,args)) =
    let args = args |> Array.map (eval vars >> toObj)
...
let run (program:instruction[]) =
    ...
    let pi = ref 0
    /// Variables    
    let variables = Dictionary<identifier, value>()
    /// Lookup For from EndFor
    let forLoops = Dictionary<index, index * identifier * expr * expr>()
    /// Lookup While from EndWhile
