...
let rec fibs() =
    ...
    LazyList.consDelayed 0 (fun () -> LazyList.consDelayed 1 (fun () -> zipWith (+) <| fibs() <| (LazyList.tail <| fibs())))
...
let rec hamming() = 
    ...
    LazyList.consDelayed 1 (fun () -> merge (LazyList.map <| ((*)2) <| hamming()) (merge  (LazyList.map ((*)3) <| hamming())  (LazyList.map ((*)5) <| hamming())))
