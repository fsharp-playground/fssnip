...
let plot name data = 
  ...
  let addLegend name color = 
    ...
    addControl box 500.0 (20.0 + float (plotCount.Value * 50))
    let block = TextBlock(FontSize = 20.0, Text = name, Width = 200.0)
    addControl block 570.0 (20.0 + float (plotCount.Value * 50))
  ...
  App.Dispatch (fun() -> 
    ...
    createLineChart color 500.0 200.0 data
    incr plotCount )

// Function that clears the window & resets count
let clear () = App.Dispatch(fun () -> 
  plotCount := 0
