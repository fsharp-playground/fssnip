...
let breakLineDetails mySeq = 
    let breakLineDetail (a,b,c:string) = 
        a,b,c.Replace(separator,"Å").Split('Å')
...
// with some nice grid-UI-control
// (results are a grouped list of errors by type)
[<EntryPoint>]
let main argv = 
    match argv with
    | [|fileName|] -> 
        //let fileName = @"C:\...\MyProgram.log"
        let results = processFile fileName
        for i in results
