...
module Price =
  ...
  let minimum = PT (fun input ->
    Math.Round(input |> Seq.map snd |> Seq.min, 2) )

  /// Gets the maximum over the whole range
  let maximum = PT (fun input ->
    Math.Round(input |> Seq.map snd |> Seq.max, 2) )
  ...
  let merge (PT f) (PT g) = PT (fun values -> f values, g values)
...
App.Dispatch (fun () -> 
  ...
  App.Console.LoadFromUrl("http://fssnip.net/raw/bH") )