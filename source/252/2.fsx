...
module ChartTypes = 
    ...
    type GenericChart() = 
        ...
        static member Create<'T when 'T :> GenericChart and 'T : (new : unit -> 'T)>(data) : 'T =
            let t = new 'T()
            t.Data <- data
            t
...
module private ChartFormUtilities = 
    ...
    let typesToClone = 
        [ typeof<DataVisualization.Charting.LabelStyle>; typeof<Axis>; typeof<Grid>; typeof<TickMark>
...
// [snippet:ChartControl]
type ChartControl(ch:GenericChart) as self = 
    inherit UserControl()
    (*[omit:(private fields and methods as in FSharpChart 0.2)]*)
    ...
    do
      ...
      let miCopyPng = new ToolStripMenuItem("Copy Png to Clipboard", 
                                            ShortcutKeys = (Keys.Control ||| Keys.C))
      let miCopyEmf = new ToolStripMenuItem("Copy Emf to Clipboard", 
                                            ShortcutKeys = (Keys.Control ||| Keys.Shift ||| Keys.C))
      let miSave = new ToolStripMenuItem("Save Image As..", 
                                         ShortcutKeys = (Keys.Control ||| Keys.S))
      let miEdit = new ToolStripMenuItem("Show Property Grid")
      let copyPngToClipboard (_) =
      ...
      let saveToFile (_) =
        let dlg = new SaveFileDialog(
                    Filter = "Bitmap (*.bmp)|*.bmp|JPEG (*.jpg)|*.jpg|EMF (*.emf)|*.emf|" + 
                    "EMF-Dual (*.emf)|*.emf|EMF+ (*.emf)|*.emf|PNG (*.png)|*.png|" +
                    "GIF (*.gif)|*.gif|TIFF (*.tif)|*.tif",
                    FilterIndex = 6, RestoreDirectory = true)
        if dlg.ShowDialog() = DialogResult.OK then
            ...
            chart.SaveImage(dlg.FileName, format)

      miEdit.Click.Add(fun _ -> 
        miEdit.Checked <- not miEdit.Checked
        props.Visible <- miEdit.Checked)
      miCopyPng.Click.Add(copyPngToClipboard)
      miCopyEmf.Click.Add(copyEmfToClipboard)
      miSave.Click.Add(saveToFile)

      menu.Items.AddRange ([ miCopyPng; miCopyEmf; miSave; miEdit ] |> Seq.cast |> Seq.toArray )
      self.ContextMenuStrip <- menu

      self.KeyDown.Add(fun e -> if e.Control = true && e.Shift = false && e.KeyCode = Keys.C then 
                                  copyPngToClipboard e)
      self.KeyDown.Add(fun e -> if e.Control = true && e.Shift = true  && e.KeyCode = Keys.C then 
                                  copyEmfToClipboard e)
      self.KeyDown.Add(fun e -> if e.Control = true && e.KeyCode = Keys.S then saveToFile e)

    member x.ChartSeries = seq { for s in series -> s }
    member x.SaveImage(filename,format) = chart.SaveImage((filename:string), (format:ChartImageFormat))
// [/snippet]

type ChartForm<'TData when 'TData :> DataSource and 'TData : (new : unit -> 'TData)>(ch:GenericChart<'TData>) as self = 
  inherit Form()
  ...
  do 
    ...
    self.KeyDown.Add(fun e -> if e.KeyCode = Keys.Escape then self.Close())


  member x.ChartControl = ctrl

// [snippet:ChartExtensions]
[<AutoOpen>]
module ChartExtensions =
  type FSharpChart with
    (*[omit:(static member Create)]*)
