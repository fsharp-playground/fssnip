module FSharp
    module IronPython = 
        ...
        let pythonEngine = Python.CreateEngine() 
        ...
        let (?) (instance : obj) member' : 'Result = 
            ...
            if FSharpType.IsFunction resultType then
                ...
                let callable = pythonEngine.Operations.GetMember(instance, member')
                if not (pythonEngine.Operations.IsCallable(callable))
                ...
                downcast FSharpValue.MakeFunction(resultType, fun argsFromInvocation -> 
                    pythonEngine.Operations.Invoke(callable, determineArgsFromInvocation(argsFromInvocation) ) ) 
            else
                downcast pythonEngine.Operations.GetMember(instance, member')
        
        let (?<-) (instance : obj) member' value = 
            pythonEngine.Operations.SetMember(instance, member', value)

        let createScriptSource (scriptText:string) = 
            pythonEngine.CreateScriptSourceFromString(scriptText, SourceCodeKind.AutoDetect)
        ...
        let addPythonPath (path:string) = 
            let sys = pythonEngine.GetSysModule()
