    ...
    let (|Young|_|) age (lst: list<Person>) =
        lst |> List.mapi(fun i _ -> (lst.Item i).Age) |> List.tryFind(fun i -> i <= age)

    //A matching funtion
    let findAYoungAge = function
    ...
    let listOfPerson = [{Name="John"; Age=29}; {Name="Simon"; Age=24}; {Name="Shirley"; Age=50}]

    //"Found a young age of 24"
    findAYoungAge listOfPerson
